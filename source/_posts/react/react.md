...
## Context原理
很多人在接触Context源码之前都会想，React数据源是从上向下传递的，Context和Redux是不是通过发布订阅模式，将所有状态存放在Provider中，然后在目标组件外包了一层订阅需要的属性更新，通过state的方式更新目标组件的props。有这种想法很合理，因为Vue中就是采用发布订阅模式更新数据源的，这也是一种脱离组件树实现跨组件共享状态的一种方式，但是在React中Context并没有这么复杂，Context本质上只是一个组件，即跟普通组件一样，就是一个简单的Json，只不过Json中的组件类型是特定的Context类型，当Context中的值发生改变时会按照React状态更新机制执行，从根节点查找需要更新的节点，组成局部更新树，然后通过异步任务去进行更新。
